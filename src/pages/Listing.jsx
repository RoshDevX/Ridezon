...
const Listing = () => {
  ...
  // get search value from Header search bar (URL query param)
  const searchFromURL = (searchParams.get("search") || "").toLowerCase().trim();
  ...
  // Search logic uses searchFromURL from Header
  const matchesSearch = (car) => {
    if (!searchFromURL) return true;
    return (
      (car.title || "").toLowerCase().includes(searchFromURL) ||
      (car.city || "").toLowerCase().includes(searchFromURL) ||
      (car.country || "").toLowerCase().includes(searchFromURL)
    );
  };

  // Form page search (destination)
          ...
          <div className="max-sm:px-10 sm:pr-10 bg-white p-4 rounded-l-xl my-4">
            {searchFromURL && (
              <h3 className="text-gray-700 mb-4">
                Showing results for:{" "}
                <span className="font-semibold">{searchFromURL}</span>
              </h3>
            )}
